...
def opening():
    print(
        f"""Current model: {str(tg_model)}
Vector DB: {db.VECTOR_ID}, User DB: {db.USER_DB_ID}
Prompt: {p.prompt_id}"""
    )


def interface(command: str) -> bool:
    match command:
        case "exit":
            exit()
        case "print":
        ...
        case "stream":
            ...
            print(f"Streamed response is now {'' if STREAM_MODE else 'in'}active.")
        case "clear":
            db.clear_cache()
            print("Cleared cache & log.")
        case "log":
            print(f"Log print is now {'on' if tg.toggle_log_print() else 'off'}.")
        case "debug":
...
def main():
    ...
    while True:
        ...
        if DEBUG_ACTIVE and interface(query.lower()):
            continue
        default(query)
